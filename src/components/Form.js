...
export class Form extends React.Component {
  ...
  checkEmailValid() {
    let emailValid = /^([\w.%+-]+)@([\w-]+\.)+([\w]{2,})$/i.test(this.state.emailText);
    this.setState({
      ...this.state,
      emailValid
    });
  ...
  checkPhoneValid() {
    const phoneValid = /^\(?([0-9]{3})\)?[-. ]?([0-9]{3})[-. ]?([0-9]{4})$/.test(this.state.phoneText);
    this.setState({
      ...this.state,
      phoneValid
    });
  ...
  checkUrlValid() {
    const urlValid = /^(http[s]?:\/\/){0,1}(www\.){0,1}[a-zA-Z0-9\.\-]+\.[a-zA-Z]{2,5}[\.]{0,1}/.test(this.state.urlText);
    this.setState({
      ...this.state,
      urlValid
    })
